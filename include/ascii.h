/* ascii.h
   General header file with defines for ASCII hex codes.
*/
/*!
...
#define ASCII_RS	0x1E     /* Record S{
              if (key==13)
                selected_line=1;
              else {
                st[0]=key;
                st[1]='\0';
                if (strlen(username)<MAXUSERNAMESIZE) {
                    //strcat(username,st);
                    l=strlen(username);
                    username[l]=key; username[l+1]='\0';
                }
              }
            }eparator*/
...
#define BACKSPACE        ASCII_BS     /* Back Space */
